...
const onValidate=(prop,isValid)=>{
  if (prop==='email')//// prop 对应表单项的 prop 属性
    isEmailValid.value=isValid // isValid 表示该字段是否通过验证
}
  ...
  emailFormRef.value.validate(isValid=>{
    if(isValid){
      ...
      get(`api/auth/ask-code?email=${emailForm.email}&type=modify`,()=>{
        ElMessage.success(`验证码发送成功到邮箱:${emailForm.email}，请注意查收`)
...
function beforeAvatarUpload(rawFile){
  if (rawFile.type!=='image/jpeg' && rawFile.type!=='image/png'){
    ElMessage.error("头像只能是jpeg和png这两种格式")
    return false;
  }
...
<template>
  <div style="display: flex;max-width: 950px;margin: auto" >
    <div class="setting-left">
      <card :v-loading="loading.form" :icon="User" title="账号信息设置" desc="在这里编辑您的个人信息，您可以在隐私设置的选择是否展示这些信息" >
      ...
      <card style="margin-top: 10px" :icon="Message" title="电子邮件设置" desc="您可以在这里修改默认绑定的电子邮件地址">
        <el-form ref="emailFormRef" :model="emailForm"  @validate="onValidate" :rules="rules" label-position="top" style=" margin: 0 10px 10px 10px ">
          ...
          <el-form-item prop="code">
           <el-row style="width: 100%" :gutter="10">
             <el-col :span="18">
                <el-input placeholder="请先获取验证码" v-model="emailForm.code"/>
             </el-col>
             <el-col :span="6">
