...
import {
  ...
  EditPen,
  Picture,
  Link, Microphone, CircleCheck, Star, FolderOpened, ArrowRight
...
function updateList(){
  ...
  get(`api/forum/list-topic?page=${topics.page}&type=${topics.type}`, data=>{
    console.log("当前数据的条数"+data.length)
    if(data){
      console.log("我更新了")
     data.forEach(d=>topics.list.push(d))
      topics.page++
      console.log("page:"+topics.page)
    }
    if(!data || data.length<10){
      console.log('数据没有了或者不够10条')
      topics.end=true
    }
  })
}
...
navigator.geolocation.getCurrentPosition(position=>{
  ...
  get('api/forum/weather?longitude=116.40529&latitude=39.9049',data=>{
    Object.assign(weather,data)
    weather.success=true
  })
},{
          ...
          <div style="position: sticky;top: 15px">
            <light-card>
              <div class="collect-list-button" @click="collects=true">
                ...
                <el-icon style="transform:  translateY(3px)"><ArrowRight/></el-icon>
              </div>
            </light-card>
