...
public class ForumController {
    ...
    @GetMapping("/weather")
    public RestBean<WeatherVO> weather(@RequestParam double longitude,@RequestParam double latitude) {
        WeatherVO  vo = service.fetchWeather(longitude, latitude);
       return  vo==null ? RestBean.failure(400,"获取地理位置和天气信息失败") : RestBean.success(vo);
    }
    @GetMapping("/types")
